import { AdsDataType } from "../Type";


  const AdsData: AdsDataType[] = [
    {
      id: "1",
      src: "https://dkstatics-public.digikala.com/digikala-adservice-banners/792b26d05b1af23cc09a37969d8739caae938b01_1704715052.gif?x-oss-process=image?x-oss-process=image/format,webp",
        title: "پارتنرشیپ-سیف",
        alt: "پارتنرشیپ-سیف"
    },
    {
        id: "2",
        src: "https://dkstatics-public.digikala.com/digikala-adservice-banners/7cfd76c867ccdee2f2ea813b421a677296e901ae_1704975147.jpg?x-oss-process=image/quality,q_95/format,webp",
        title: "پارتنرشیپ-سروطلایی",
        alt: "پارتنرشیپ-سروطلایی"
    },

  ];
  export default AdsData;