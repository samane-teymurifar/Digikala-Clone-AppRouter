...
function DigikalaOffer() {
    ...
    <section className="mt-6 mx-48">
        ...
        {isArray(NumberedDigikalaOfferData).map((DigikalaOfferCard:any , index) => {
        return (
            <div className={`flex flex-col items-center ${index !== 6   &&  'border-l'} ${index <= 6 && 'border-b'}  border-gray-10 py-6`}>
            <Image src={DigikalaOfferCard.images.main} alt={DigikalaOfferCard.title_fa} width={60} height={60} key={`${index}-DigikalaOfferCard`}/>
            <span className="text-sm text-common-black text-ellipsis w-20 overflow-hidden whitespace-nowrap block">{DigikalaOfferCard.title_fa}</span>
            </div>
        );
        })} 
        </div>
